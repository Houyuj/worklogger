...
class AnnualGoal(Base):
    ...
    tasks = relationship('Task', back_populates='annual_goal')
...
class MonthlyGoal(Base):
    ...
    weekly_goals = relationship('WeeklyGoal', back_populates='monthly_goal')
...
class WeeklyGoal(Base):
    ...
    monthly_goal_id = Column(Integer, ForeignKey('monthly_goals.id'), nullable=True, index=True)
...
class Task(Base):
    ...
    # Four-level work-plan relationships. Values are denormalized for fast,
    # stable reporting and validated against the hierarchy in crud.py.
    project_id = Column(Integer, ForeignKey('projects.id'), nullable=True, index=True)
    annual_goal_id = Column(Integer, ForeignKey('annual_goals.id'), nullable=True, index=True)
    ...
    workflow_status = Column(String(20), nullable=False, default='unclassified')
    is_inbox = Column(Boolean, nullable=False, default=False, index=True)

    project = relationship('Project', back_populates='tasks')
    annual_goal = relationship('AnnualGoal', back_populates='tasks')

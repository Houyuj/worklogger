...
def get_projects(year: Optional[int] = None, include_archived: bool = False, db: Session = Depends(get_db)):
    return crud.get_projects(db, year=year, include_archived=include_archived)
...
def get_annual_goals(
    ...
    return crud.get_annual_goals(db, project_id, year, include_archived)
...
def get_monthly_goals(
    ...
    return crud.get_monthly_goals(db, annual_goal_id, include_archived)
...
def get_weekly_goals(
    ...
    return crud.get_weekly_goals(db, monthly_goal_id=monthly_goal_id, year=year, include_archived=include_archived)

@app.patch("/api/weekly-goals/{goal_id}", response_model=schemas.WeeklyGoal)
def update_weekly_goal(
    goal_id: int,
    goal_update: schemas.WeeklyGoalUpdate,
    db: Session = Depends(get_db)
):
    try:
        goal = crud.update_weekly_goal(db, goal_id, goal_update)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    if not goal:
...
def get_tasks(
    ...
    return crud.get_tasks(db, date_obj, completed, inbox)
...
def export_monthly_pdf(month: int = Query(...), year: int = Query(...), db: Session = Depends(get_db)):
    ...
    tasks = crud.get_tasks(db, inbox=False)
...
def generate_daily_word(date_obj: datetime, db: Session):
    ...
    tasks = crud.get_tasks(db, date=date_obj, inbox=False)
